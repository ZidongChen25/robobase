...
from robobase.backends.torch.models.encoder import EncoderModule, ResNetEncoder
from robobase.backends.torch.models.fully_connected import (
    FullyConnectedModule,
    MLPWithBottleneckFeaturesAndSequenceOutput,
    jax_style_weight_init,
)
from robobase.backends.torch.models.fusion import FusionModule, FusionMultiCamFeature
...
class BC(Method):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.obs_layout = bc_observation_layout(self.observation_space)
    ...
    def low_dim_size(self) -> int:
        return self.obs_layout.low_dim_size

    def build_actor(self):
        if self.actor_model is None:
            if self.model_spec is None:
                raise ValueError("BC requires either actor_model or model spec.")
            actor_spec = self.model_spec.actor_model
            if actor_spec.type != "mlp_bottleneck_sequence":
                raise NotImplementedError(
                    f"Unsupported torch BC actor model type '{actor_spec.type}'."
                )
            self.actor_model = partial(
                MLPWithBottleneckFeaturesAndSequenceOutput,
                num_rnn_layers=actor_spec.num_rnn_layers,
                rnn_hidden_size=actor_spec.rnn_hidden_size,
                keys_to_bottleneck=list(actor_spec.keys_to_bottleneck),
                bottleneck_size=actor_spec.bottleneck_size,
                norm_after_bottleneck=actor_spec.norm_after_bottleneck,
                tanh_after_bottleneck=actor_spec.tanh_after_bottleneck,
                mlp_nodes=list(actor_spec.hidden_dims),
                output_sequence_network_type=actor_spec.output_sequence_network_type,
                output_sequence_length=actor_spec.output_sequence_length,
            )
        ...
        self.actor_model.apply(jax_style_weight_init)
    ...
    def build_encoder(self):
        if self.use_pixels:
            if self.encoder_model is None:
                if self.model_spec is None or self.model_spec.encoder_model is None:
                    raise ValueError("Pixel BC requires encoder_model or model spec.")
                encoder_spec = self.model_spec.encoder_model
                if encoder_spec.type != "resnet":
                    raise NotImplementedError(
                        f"Unsupported torch BC encoder model type '{encoder_spec.type}'."
                    )
                self.encoder_model = partial(ResNetEncoder, model=encoder_spec.model)
            assert self.obs_layout.rgb_input_shape is not None
            self.encoder = self.encoder_model(input_shape=self.obs_layout.rgb_input_shape)
    ...
    def build_view_fusion(self):
        ...
        if self.use_multicam_fusion:
            if self.view_fusion_model is None and self.model_spec is not None:
                view_fusion_spec = self.model_spec.view_fusion_model
                if view_fusion_spec is not None:
                    if view_fusion_spec.type != "multicam_feature":
                        raise NotImplementedError(
                            "Unsupported torch BC view fusion model type "
                            f"'{view_fusion_spec.type}'."
                        )
                    self.view_fusion_model = partial(
                        FusionMultiCamFeature,
                        mode=view_fusion_spec.mode,
                    )
