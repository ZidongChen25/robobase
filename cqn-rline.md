...
## 消融三连 + 方差清算(2026-08-19 晨)
参照:官方配方 sandwich 同分布抽签 {41,61,63,63.5,70,90}(历史4 + 安慰剂对2,
安慰剂身份经 stop-gradient/梯度裁剪 null/obs_std 键三重审计确认;同条件对
41/90 独立支撑方差结论)。塑形 pot 对 {47,82} 同样铺满带宽 → 塑形项中性。
| 臂 | 终点(100k/101k 各50) | 判决 |
|---|---|---|
| duel0 s1(dueling关) | 62/50→56 | 带内,无大效应;dueling 在 hinge 在场时非承重墙(懒路只在 λ=0 崩溃时致命,与 A 流解剖一致) |
| mclbsw s1(mclb在线×sandwich) | 58/50→54 | 带内偏低;交叉格关闭:mclb 在线无拯救,与 m1 (move_plate 51/65) 同向 |
| sup01 s1/s2(C51 [0,1]) | 36/36 + 50/56 → 均值44.5 | 两 seed 均低于官方均值,s1 低于最低抽签;分辨率×4 假设行为层未兑现,候选机制:负区不是浪费 —— margin 下压需要空间,[0,1] 里 sibling 压到 0 触底 → margin 变薄(与 Wave-1 rfloor −26.25 尸检同构) |
方法论结论:sandwich 官方抽签带宽 ~49 点,单旋钮×1-2 seed 的消融只能读
"灾难/奇迹"级效应;本轮四臂全部落带内或略低 → 全部关闭,不补 seed。
资源转向:机制探针(便宜高信息)+ 臂 A 合成(in-sample 目标+活掩码解码,
针对已证机制),判读用配对协议或换低方差读出。
