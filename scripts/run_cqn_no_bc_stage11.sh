#!/usr/bin/env bash
# One-GPU Stage-11 replication: ordered success-return seeds 1 and 2.
set -euo pipefail
cd "$(dirname "$0")/.."

GPU="${1:-0}"
STAMP="$(date +%Y%m%d%H%M%S)"
BASE="exp_local/cqn_no_bc/stage11_ordered_success_gpu${GPU}_${STAMP}"
SEED1="${BASE}/ordered_success_seed1"
SEED2="${BASE}/ordered_success_seed2"
...
printf "%s\n" "${BASE}" > exp_local/cqn_no_bc/stage11_latest.txt
...
train_one 1 "${SEED1}" &
SEED1_PID=$!
printf "%s\n" "${SEED1_PID}" > "${BASE}/ordered_success_seed1.pid"
sleep 120
train_one 2 "${SEED2}" &
SEED2_PID=$!
printf "%s\n" "${SEED2_PID}" > "${BASE}/ordered_success_seed2.pid"
...
.venv/bin/python scripts/summarize_cqn_no_bc_stage11.py \
  --baseline-seed1 "${BASELINE1}" \
  --baseline-seed2 "${BASELINE2}" \
  --treatment-seed1 "${SEED1}" \
  --treatment-seed2 "${SEED2}" \
  --output "${BASE}/stage11_summary.json" \
  > "${BASE}/stage11_summary.log" 2>&1
touch "${BASE}/complete"
