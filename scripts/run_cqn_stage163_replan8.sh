#!/usr/bin/env bash
# Stage-163: receding-horizon (replan-8) training arm on GPU5, watcher on
# GPU3 (coexists with whatever trains there).
set -euo pipefail
cd "$(dirname "$0")/.."
...
mkdir -p "${BASE}"
RUN_DIR="${BASE}/move_plate_replan8_seed1_gpu5_${STAMP}"

echo "[163] train replan8 on GPU5"
MUJOCO_GL=egl CUDA_VISIBLE_DEVICES=5 MUJOCO_EGL_DEVICE_ID=5 \
  .venv/bin/python train_fast.py \
  launch=cqn_as_pixel_bigym_stage163_replan8 \
...
XLA_PYTHON_CLIENT_PREALLOCATE=false MUJOCO_GL=egl nohup \
  ...
  --run-dir "${RUN_DIR}" --gpu-id 3 --num-episodes 50 --eval-seed-start 400 \
  > "${RUN_DIR}.watcher.log" 2>&1 &
wait "${TRAIN_PID}"
echo "[163] done train replan8"
...
MUJOCO_GL=egl .venv/bin/python scripts/analyze_cqn_branch_counterfactual.py \
  ...
  --output "${BASE}/sibling_replan8_seed1.json" --gpu-id 5 \
  ...
  --bootstrap-replicates 10000 > "${BASE}/sibling_replan8_seed1.log" 2>&1
echo "[163] probe done"
...
echo "[163] eval200 done"
echo "[163] all complete"
