#!/usr/bin/env python3
"""Select the Stage-10 binary episodic-success Q treatment."""
...
def summarize(control_dir: Path, treatment_dir: Path) -> dict:
    arms = {
        "discounted_dense_control": _arm(control_dir),
        "episodic_success_q": _arm(treatment_dir),
    }
    control = arms["discounted_dense_control"]
    treatment = arms["episodic_success_q"]
    delta = treatment["best_success"] - control["best_success"]
    gate_pass = treatment["best_success"] >= 0.60 and delta >= 0.15
    treatment_wins = treatment["best_success"] > control["best_success"]
    selected_name = (
        "episodic_success_q"
        if treatment_wins
        else "discounted_dense_control"
    )
    return {
        "protocol": {
            ...
            "gate": (
                "episodic-success best >=60% and improvement >=15pp"
            ),
        },
        "arms": arms,
        "contrasts": {
            "episodic_success_minus_discounted_dense": delta,
        },
        "selected_variant": selected_name,
        "selected_step": arms[selected_name]["best_step"],
        "selected_success": arms[selected_name]["best_success"],
        "episodic_success_gate": "pass" if gate_pass else "fail",
    }
