#!/usr/bin/env bash
# One-GPU Stage-20 replication: gap-increasing Bellman seeds 1 and 2.
set -euo pipefail
cd "$(dirname "$0")/.."

GPU="${1:-0}"
STAMP="$(date +%Y%m%d%H%M%S)"
BASE="exp_local/cqn_no_bc/stage20_advantage_gap_gpu${GPU}_${STAMP}"
SEED1="${BASE}/advantage_gap_seed1"
SEED2="${BASE}/advantage_gap_seed2"
...
printf "%s\n" "${BASE}" > exp_local/cqn_no_bc/stage20_latest.txt
...
train_one () {
  ...
  XLA_PYTHON_CLIENT_PREALLOCATE=true \
    ...
    launch=cqn_as_pixel_bigym_nobc_stage20_advantage_gap_gate \
...
train_one 1 "${SEED1}" &
SEED1_PID=$!
printf "%s\n" "${SEED1_PID}" > "${BASE}/advantage_gap_seed1.pid"
sleep 120
train_one 2 "${SEED2}" &
SEED2_PID=$!
printf "%s\n" "${SEED2_PID}" > "${BASE}/advantage_gap_seed2.pid"
...
.venv/bin/python scripts/summarize_cqn_no_bc_stage20.py \
  --baseline-seed1 "${BASELINE1}" \
  --baseline-seed2 "${BASELINE2}" \
  --treatment-seed1 "${SEED1}" \
  --treatment-seed2 "${SEED2}" \
  --output "${BASE}/stage20_summary.json" \
  > "${BASE}/stage20_summary.log" 2>&1
touch "${BASE}/complete"
