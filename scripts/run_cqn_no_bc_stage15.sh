#!/usr/bin/env bash
# One-GPU Stage-15 replication: replay-SARSA dense-Q seeds 1 and 2.
set -euo pipefail
cd "$(dirname "$0")/.."

GPU="${1:-0}"
STAMP="$(date +%Y%m%d%H%M%S)"
BASE="exp_local/cqn_no_bc/stage15_replay_sarsa_gpu${GPU}_${STAMP}"
SEED1="${BASE}/replay_sarsa_seed1"
SEED2="${BASE}/replay_sarsa_seed2"
...
printf "%s\n" "${BASE}" > exp_local/cqn_no_bc/stage15_latest.txt
...
train_one () {
  ...
  XLA_PYTHON_CLIENT_PREALLOCATE=true \
    ...
    launch=cqn_as_pixel_bigym_nobc_stage15_replay_sarsa_gate \
...
train_one 1 "${SEED1}" &
SEED1_PID=$!
printf "%s\n" "${SEED1_PID}" > "${BASE}/replay_sarsa_seed1.pid"
sleep 120
train_one 2 "${SEED2}" &
SEED2_PID=$!
printf "%s\n" "${SEED2_PID}" > "${BASE}/replay_sarsa_seed2.pid"
...
.venv/bin/python scripts/summarize_cqn_no_bc_stage15.py \
  --baseline-seed1 "${BASELINE1}" \
  --baseline-seed2 "${BASELINE2}" \
  --treatment-seed1 "${SEED1}" \
  --treatment-seed2 "${SEED2}" \
  --output "${BASE}/stage15_summary.json" \
  > "${BASE}/stage15_summary.log" 2>&1
touch "${BASE}/complete"
