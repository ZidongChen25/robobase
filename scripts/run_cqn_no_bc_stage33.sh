#!/usr/bin/env bash
# Stage 33: episode-persistent twin-head exploration, reward-only offline->online.
set -euo pipefail
cd "$(dirname "$0")/.."

GPU="${1:-3}"
STAGE32_BASE="${2:-$(tr -d '\n' < exp_local/cqn_no_bc/stage32_latest.txt)}"
if [[ ! -f "${STAGE32_BASE}/stage32_summary.json" ]]; then
  echo "Stage-32 summary is required: ${STAGE32_BASE}/stage32_summary.json" >&2
  exit 2
fi

STAMP="$(date +%Y%m%d%H%M%S)"
BASE="exp_local/cqn_no_bc/stage33_episodic_twin_explore_gpu${GPU}_${STAMP}"
LAUNCH="cqn_as_pixel_bigym_nobc_stage33_episodic_twin_explore_gate"
...
printf "%s\n" "${BASE}" > exp_local/cqn_no_bc/stage33_latest.txt
printf "%s\n" "${STAGE32_BASE}" > "${BASE}/stage32_baseline.txt"
...
run_seed () {
  local seed="$1"
  local run_dir="${BASE}/offline_episodic_twin_seed${seed}"
  local status=0
...
# One card carries two matched seeds. Stagger compilation to avoid a transient
# full-card memory spike while preserving concurrent training thereafter.
run_seed 1 &
SEED1_PID=$!
...
.venv/bin/python -m scripts.summarize_cqn_no_bc_stage33 \
  --stage32-summary "${STAGE32_BASE}/stage32_summary.json" \
  --explore-seed1 "${BASE}/offline_episodic_twin_seed1" \
  --explore-seed2 "${BASE}/offline_episodic_twin_seed2" \
  --output "${BASE}/stage33_summary.json" \
  > "${BASE}/stage33_summary.log" 2>&1
touch "${BASE}/complete"
