#!/usr/bin/env bash
# Agent line Stage A18: GPT Stage-42 recipe x de-saturation. One seed per
...
BASE="exp_local/cqn_no_bc/agent_a18_s${SEED}_gpu${GPU}_${STAMP}"
RUN="${BASE}/run"
mkdir -p "${BASE}"
printf '%s\n' "${BASE}" > "exp_local/cqn_no_bc/agent_a18_s${SEED}_latest.txt"

XLA_PYTHON_CLIENT_PREALLOCATE=false MUJOCO_GL=egl \
  .venv/bin/python train_fast.py \
  launch=cqn_as_pixel_bigym_nobc_agent_a18_offline_gate \
...
XLA_PYTHON_CLIENT_PREALLOCATE=false MUJOCO_GL=egl \
  .venv/bin/python train_fast.py \
  launch=cqn_as_pixel_bigym_nobc_agent_a18_online_gate \
  env=bigym/move_plate seed="${SEED}" \
  batch_size=256 demo_batch_size=256 \
  num_pretrain_steps="${OFFLINE_UPDATES}" \
...
XLA_PYTHON_CLIENT_PREALLOCATE=false MUJOCO_GL=egl \
  ...
  --csv-name val50_seeds400_a18.csv > "${BASE}/val50.log" 2>&1
touch "${BASE}/complete"
