#!/usr/bin/env python3
"""Validation-select and confirm a return-flow action readout.

This gate is for single-tower return-sample CQN-Flow checkpoints whose native
policy is already value-selected.  Validation independently selects the best
native R-action/mean baseline checkpoint and the best higher-sample
mean/entropic candidate.  Only a positive predeclared validation improvement
opens a disjoint confirmation split.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--validation-seed-start", type=int, default=44000)
    parser.add_argument("--confirmation-episodes", type=int, default=100)
    parser.add_argument("--confirmation-seed-start", type=int, default=45000)
    ...
    for order, (snapshot_label, snapshot) in enumerate(args.snapshot):
        variants.append(
            Variant(
                label=(
                    f"{snapshot_label}_r"
                    f"{args.baseline_action_flow_samples}_mean"
                ),
                snapshot_label=snapshot_label,
                snapshot=snapshot,
                action_flow_samples=args.baseline_action_flow_samples,
                aggregation="mean",
                temperature=1.0,
                kind="baseline",
        ...
        variants.append(
            Variant(
                label=(
                    f"{snapshot_label}_r"
                    f"{args.candidate_action_flow_samples}_mean"
                ),
                snapshot_label=snapshot_label,
                snapshot=snapshot,
                action_flow_samples=args.candidate_action_flow_samples,
                aggregation="mean",
                temperature=1.0,
                kind="candidate",
                snapshot_order=order,
        ...
        for temperature in args.temperatures:
            variants.append(
                Variant(
                    label=(
                        f"{snapshot_label}_r"
                        f"{args.candidate_action_flow_samples}_entropic_"
                        f"eta{temperature:g}"
                    ),
                    snapshot_label=snapshot_label,
                    snapshot=snapshot,
...
def _run_variant(
    *,
    ...
    command = [
        ...
        str(variant.action_flow_samples),
        "--return-sample-aggregation",
        variant.aggregation,
        "--return-sample-temperature",
        f"{variant.temperature:g}",
    ]
...
def select_variant(
    ...
    *,
    ...
    def key(variant: Variant) -> tuple[float, int, float, int]:
        # On ties prefer mean over a risk-transformed readout, then the larger
        # entropic temperature (closer to mean), then the earlier checkpoint.
        return (
            _success(payloads[variant.label]),
            int(variant.aggregation == "mean"),
            variant.temperature,
...
def paired_row(
    ...
    *,
    ...
    baseline_payload = payloads[baseline.label]
    candidate_payload = payloads[candidate.label]
    baseline_by_seed = {
    ...
    return {
        ...
        "action_flow_samples": int(candidate.action_flow_samples),
...
def _variant_dict(variant: Variant) -> dict:
    return {
        ...
        "action_flow_samples": int(variant.action_flow_samples),
...
def run_gate(args: argparse.Namespace) -> dict:
    ...
    if args.num_flow_steps < 1:
        raise ValueError("num-flow-steps must be positive")
    if args.baseline_action_flow_samples < 1:
        raise ValueError("baseline-action-flow-samples must be positive")
    if args.candidate_action_flow_samples < 1:
        raise ValueError("candidate-action-flow-samples must be positive")
    if (
        args.candidate_action_flow_samples
        <= args.baseline_action_flow_samples
    ):
        raise ValueError(
            "candidate-action-flow-samples must exceed the baseline"
        )
    ...
    selected_validation = next(
        row for row in validation_rows if row["label"] == candidate.label
    )
    ...
    validation_summary_path = (
        args.output_dir / "validation" / "summary.json"
    )
    ...
    payload = {
        ...
        "selection_rule": (
            ...
            "success; ties prefer mean, larger eta, then earlier snapshot"
        ),
    ...
    if passed_validation:
        ...
        (args.output_dir / "confirmation" / "summary.json").write_text(
            json.dumps(confirmation_summary, indent=2, sort_keys=True) + "\n"
        )
        payload.update(
            {
