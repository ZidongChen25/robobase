...
BASE="exp_local/cqn_no_bc/agent_a11_bc_gpu${GPU}_${STAMP}"
LAUNCH="cqn_as_pixel_bigym_stage36_offline_bc256_control"
...
printf '%s\n' "${BASE}" > exp_local/cqn_no_bc/agent_a11_bc_latest.txt
printf '%s\n' "${GPU}" > "${BASE}/gpu.txt"
...
train_offline () {
  local seed="$1"
  local mixed_flag="env.episode_length=3000"
  if [[ "${seed}" == "1" ]]; then
    mixed_flag="+env.treat_all_demos_as_expert=true"
  fi
  local dir
  dir="$(run_dir "${seed}")"
  XLA_PYTHON_CLIENT_PREALLOCATE=false MUJOCO_GL=egl \
    .venv/bin/python train_fast.py \
    launch="${LAUNCH}" env=bigym/move_plate "${mixed_flag}" env.expected_successful_demos=null seed="${seed}" \
...
train_offline 1 &
seed1_pid=$!
printf '%s\n' "${seed1_pid}" > "${BASE}/descale_seed1/offline.pid"
sleep 120
train_offline 2 &
seed2_pid=$!
printf '%s\n' "${seed2_pid}" > "${BASE}/descale_seed2/offline.pid"
...
evaluate () {
  ...
  XLA_PYTHON_CLIENT_PREALLOCATE=false MUJOCO_GL=egl \
    ...
    --num-eval-envs 25 \
    --only-steps "5000,7500,10000" \
    --csv-name val50_seeds400_offline.csv > "${log}" 2>&1
}

evaluate "$(run_dir 1)" "${BASE}/descale_seed1/val50.log" &
eval1=$!
evaluate "$(run_dir 2)" "${BASE}/descale_seed2/val50.log" &
eval2=$!
status=0
wait "${eval1}" || status=$?
eval2_status=0
