...
BASE="exp_local/cqn_no_bc/agent_a11_nobc_mixed_gpu${GPU}_${STAMP}"
...
printf '%s\n' "${BASE}" > exp_local/cqn_no_bc/agent_a11_nobc_latest.txt
printf '%s\n' "${GPU}" > "${BASE}/gpu.txt"
...
train_offline () {
  ...
  XLA_PYTHON_CLIENT_PREALLOCATE=false MUJOCO_GL=egl \
    ...
    launch="${LAUNCH}" env=bigym/move_plate +env.treat_all_demos_as_expert=true env.expected_successful_demos=null seed="${seed}" \
