#!/usr/bin/env bash
# One-GPU Stage-17 replication: corrected replay-SARSA dense-Q seeds 1 and 2.
set -euo pipefail
cd "$(dirname "$0")/.."

GPU="${1:-0}"
STAMP="$(date +%Y%m%d%H%M%S)"
BASE="exp_local/cqn_no_bc/stage17_effective_replay_sarsa_gpu${GPU}_${STAMP}"
SEED1="${BASE}/effective_replay_sarsa_seed1"
SEED2="${BASE}/effective_replay_sarsa_seed2"
...
printf "%s\n" "${BASE}" > exp_local/cqn_no_bc/stage17_latest.txt
...
train_one () {
  ...
  XLA_PYTHON_CLIENT_PREALLOCATE=true \
    ...
    launch=cqn_as_pixel_bigym_nobc_stage17_effective_replay_sarsa_gate \
...
train_one 1 "${SEED1}" &
SEED1_PID=$!
printf "%s\n" "${SEED1_PID}" > "${BASE}/effective_replay_sarsa_seed1.pid"
sleep 120
train_one 2 "${SEED2}" &
SEED2_PID=$!
printf "%s\n" "${SEED2_PID}" > "${BASE}/effective_replay_sarsa_seed2.pid"
...
.venv/bin/python scripts/summarize_cqn_no_bc_stage17.py \
  --baseline-seed1 "${BASELINE1}" \
  --baseline-seed2 "${BASELINE2}" \
  --treatment-seed1 "${SEED1}" \
  --treatment-seed2 "${SEED2}" \
  --output "${BASE}/stage17_summary.json" \
  > "${BASE}/stage17_summary.log" 2>&1
touch "${BASE}/complete"
