#!/usr/bin/env bash
# Agent line Stage A7: paired label-smoothing online arms on GPU 5.
...
LAUNCH="cqn_as_pixel_bigym_nobc_agent_a7_smooth_gate"
OFFLINE_UPDATES=10000
GLOBAL_LIMIT=20000
STAMP="$(date +%Y%m%d%H%M%S)"
BASE="exp_local/cqn_no_bc/agent_a7_smooth_gpu${GPU}_${STAMP}"

mkdir -p "${BASE}"
printf '%s\n' "${BASHPID}" > "${BASE}/controller.pid"
printf '%s\n' "${BASE}" > exp_local/cqn_no_bc/agent_a7_latest.txt
