#!/usr/bin/env bash
# Continue the qualified Stage-41 pair from raw 20k to raw 30k only.
set -euo pipefail
cd "$(dirname "$0")/.."

BASE="${1:-$(tr -d '\n' < exp_local/cqn_no_bc/stage41_latest.txt)}"
GPU="${2:-$(tr -d '\n' < "${BASE}/gpu.txt")}"
STAGE38="$(tr -d '\n' < "${BASE}/stage38_source.txt")"
...
test -s "${BASE}/stage41_summary.json"
if ! rg -q '"eligible_for_bounded_raw30_extension": true' \
    "${BASE}/stage41_summary.json"; then
  printf 'Stage-41 initial gate did not authorize raw30.\n' >&2; exit 2
fi
if [[ -e "${BASE}/extension_complete" ]]; then exit 0; fi
for seed in 1 2; do test -s "$(run_dir "${seed}")/snapshots/20000_snapshot.pkl"; done

printf '%s\n' "${BASHPID}" > "${BASE}/extension_controller.pid"
printf '%s objective=stage41_raw30_scaling full_run=false heldout=sealed\n' \
  "$(date --iso-8601=seconds)" > "${BASE}/extension_protocol_registered.txt"

timeout 30s env CUDA_VISIBLE_DEVICES="${GPU_UUID}" JAX_PLATFORMS=cuda \
  .venv/bin/python -c \
  "import jax; d=jax.devices(); assert d and d[0].platform == 'gpu', d" \
  > "${BASE}/extension_cuda_probe.log" 2>&1
timeout 30s env CUDA_VISIBLE_DEVICES="${GPU_UUID}" MUJOCO_GL=egl \
  ...
  > "${BASE}/extension_egl_probe.log" 2>&1
...
train_seed () {
  ...
  env CUDA_VISIBLE_DEVICES="${GPU_UUID}" JAX_PLATFORMS=cuda \
    ...
    num_train_frames=30000 replay.demo_only_updates=false \
    ...
    > "${BASE}/seed${seed}_extension_train.log" 2>&1
}
...
if ! kill -0 "${seed1_pid}" 2>/dev/null; then
  wait "${seed1_pid}" || status=$?; printf '%s\n' "${status}" > "${BASE}/extension_training_failed"; exit "${status}"
fi
train_seed 2 & seed2_pid=$!
touch "${BASE}/extension_training_pair_started"
...
for seed in 1 2; do
  run="$(run_dir "${seed}")"
  for step in 22500 25000 27500 30000; do test -s "${run}/snapshots/${step}_snapshot.pkl"; done
done
...
evaluate_seed () {
  ...
  env CUDA_VISIBLE_DEVICES="${GPU_UUID}" JAX_PLATFORMS=cuda \
    ...
    --csv-name val50_seeds400_stage41_extension.csv \
    > "${BASE}/seed${seed}_extension_val50.log" 2>&1
}
...
if [[ "${status}" -ne 0 ]]; then
  printf '%s\n' "${status}" > "${BASE}/extension_validation_failed"; exit "${status}"
fi
touch "${BASE}/extension_validation_complete"
.venv/bin/python scripts/summarize_cqn_no_bc_stage41_extension.py \
  --stage-dir "${BASE}" --stage38-dir "${STAGE38}" \
  --output "${BASE}/stage41_extension_summary.json" \
  > "${BASE}/stage41_extension_summary.log" 2>&1
