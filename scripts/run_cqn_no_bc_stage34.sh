#!/usr/bin/env bash
# Stage 34: joint top-two beam rollout search, reward-only offline->online.
set -euo pipefail
cd "$(dirname "$0")/.."

GPU="${1:-3}"
STAGE33_BASE="${2:-$(tr -d '\n' < exp_local/cqn_no_bc/stage33_latest.txt)}"
if [[ ! -f "${STAGE33_BASE}/stage33_summary.json" ]]; then
  echo "Stage-33 summary is required: ${STAGE33_BASE}/stage33_summary.json" >&2
  exit 2
fi

STAMP="$(date +%Y%m%d%H%M%S)"
BASE="exp_local/cqn_no_bc/stage34_joint_beam_gpu${GPU}_${STAMP}"
LAUNCH="cqn_as_pixel_bigym_nobc_stage34_joint_beam_gate"
...
printf "%s\n" "${BASE}" > exp_local/cqn_no_bc/stage34_latest.txt
printf "%s\n" "${STAGE33_BASE}" > "${BASE}/stage33_baseline.txt"
...
run_seed () {
  local seed="$1"
  local run_dir="${BASE}/offline_joint_beam_seed${seed}"
  local status=0
...
# One card carries two matched seeds; stagger first compilation only.
run_seed 1 &
SEED1_PID=$!
...
.venv/bin/python -m scripts.summarize_cqn_no_bc_stage34 \
  --stage33-summary "${STAGE33_BASE}/stage33_summary.json" \
  --beam-seed1 "${BASE}/offline_joint_beam_seed1" \
  --beam-seed2 "${BASE}/offline_joint_beam_seed2" \
  --output "${BASE}/stage34_summary.json" \
  > "${BASE}/stage34_summary.log" 2>&1
touch "${BASE}/complete"
