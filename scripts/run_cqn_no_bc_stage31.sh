#!/usr/bin/env bash
# Stage 31: direct per-bin C51 head under the Stage-30 offline->online protocol.
set -euo pipefail
cd "$(dirname "$0")/.."

GPU="${1:-1}"
STAGE30_BASE="${2:-$(tr -d '\n' < exp_local/cqn_no_bc/stage30_latest.txt)}"
if [[ ! -f "${STAGE30_BASE}/stage30_summary.json" ]]; then
  echo "Stage-30 summary is required: ${STAGE30_BASE}/stage30_summary.json" >&2
  exit 2
fi

STAMP="$(date +%Y%m%d%H%M%S)"
BASE="exp_local/cqn_no_bc/stage31_offline_direct_head_gpu${GPU}_${STAMP}"
LAUNCH="cqn_as_pixel_bigym_nobc_stage31_offline_direct_head_gate"
...
printf "%s\n" "${BASE}" > exp_local/cqn_no_bc/stage31_latest.txt
printf "%s\n" "${STAGE30_BASE}" > "${BASE}/stage30_baseline.txt"
...
run_seed () {
  local seed="$1"
  local run_dir="${BASE}/offline_direct_head_seed${seed}"
  local status=0
...
# One physical card, two independent seeds. Stagger only their compilations.
run_seed 1 &
SEED1_PID=$!
...
.venv/bin/python -m scripts.summarize_cqn_no_bc_stage31 \
  --stage30-summary "${STAGE30_BASE}/stage30_summary.json" \
  --direct-seed1 "${BASE}/offline_direct_head_seed1" \
  --direct-seed2 "${BASE}/offline_direct_head_seed2" \
  --output "${BASE}/stage31_summary.json" \
  > "${BASE}/stage31_summary.log" 2>&1
touch "${BASE}/complete"
