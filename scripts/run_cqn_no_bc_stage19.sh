#!/usr/bin/env bash
# One-GPU Stage-19 replication: reward-scaled dense C51 seeds 1 and 2.
set -euo pipefail
cd "$(dirname "$0")/.."

GPU="${1:-0}"
STAMP="$(date +%Y%m%d%H%M%S)"
BASE="exp_local/cqn_no_bc/stage19_reward_scale_gpu${GPU}_${STAMP}"
SEED1="${BASE}/reward_scale_seed1"
SEED2="${BASE}/reward_scale_seed2"
...
printf "%s\n" "${BASE}" > exp_local/cqn_no_bc/stage19_latest.txt
...
train_one 1 "${SEED1}" &
SEED1_PID=$!
printf "%s\n" "${SEED1_PID}" > "${BASE}/reward_scale_seed1.pid"
sleep 120
train_one 2 "${SEED2}" &
SEED2_PID=$!
printf "%s\n" "${SEED2_PID}" > "${BASE}/reward_scale_seed2.pid"
...
.venv/bin/python scripts/summarize_cqn_no_bc_stage19.py \
  --baseline-seed1 "${BASELINE1}" \
  --baseline-seed2 "${BASELINE2}" \
  --treatment-seed1 "${SEED1}" \
  --treatment-seed2 "${SEED2}" \
  --output "${BASE}/stage19_summary.json" \
  > "${BASE}/stage19_summary.log" 2>&1
touch "${BASE}/complete"
