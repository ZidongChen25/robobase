...
if [[ "${status}" -ne 0 ]]; then
  printf '%s\n' "${status}" > "${BASE}/a10b_offline_failed"
  exit "${status}"
fi
for seed in 1 2; do
  dir="$(run_dir "${seed}")"
...
touch "${BASE}/a10b_offline_complete"
...
evaluate "$(run_dir 1)" "${BASE}/descale_seed1/a10b_val50.log" &
eval1=$!
evaluate "$(run_dir 2)" "${BASE}/descale_seed2/a10b_val50.log" &
eval2=$!
status=0
wait "${eval1}" || status=$?
eval2_status=0
...
if [[ "${status}" -ne 0 ]]; then
  printf '%s\n' "${status}" > "${BASE}/a10b_validation_failed"
  exit "${status}"
fi
touch "${BASE}/a10b_validation_complete"
touch "${BASE}/a10b_complete"
