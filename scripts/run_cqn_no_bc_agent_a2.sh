#!/usr/bin/env bash
# Agent line Stage A2: online continuation of the de-saturated offline
...
BASE="exp_local/cqn_no_bc/agent_a2_descale_online_gpu${GPU}_${STAMP}"

test -f "${A1_BASE}/a1b_complete"
mkdir -p "${BASE}"
printf '%s\n' "${BASHPID}" > "${BASE}/controller.pid"
printf '%s\n' "${BASE}" > exp_local/cqn_no_bc/agent_a2_latest.txt
